...
episode_reward = open('episode_r', 'w', 1)
class Link():
...
class Network():
    ...
    def run_for_dur(self, dur):
        ...
        # Very high thpt
        reward = (1e-5 * throughput / (8 * BYTES_PER_PACKET) - 1e7 * latency - 2e3 * loss)
        # print("Reward = %f, thpt = %f, lat = %f, loss = %f" % (reward, throughput / (8 * BYTES_PER_PACKET), 1e7 * latency, loss))
...
class Sender():
    ...
    # rate-based update method, we won't use it.
    ...
    def set_cwnd(self, new_cwnd):
        ...
        # It's a intuition, the lower bound and the upper bound is difficult to determine.
    ...
    def reset(self):
        ...
        # TODO reset cwnd to the initial value
        self.bytes_in_flight = 0
        self.min_latency = None
        self.reset_obs()
        self.history = sender_obs.SenderHistory(self.history_len,
...
class SimulatedNetworkEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        for i in range(0, 1):  # len(actions)):
            ...
            action = actions
            # simultaneously modify sending rate and cwnd
    ...
    def reset(self):
        ...
        print("{}\t{}".format(self.reward_sum, self.reward_ewma), file=episode_reward)
        self.reward_sum = 0.0
...
register(id='PccNs-v1', entry_point='dc_sim:SimulatedNetworkEnv')
