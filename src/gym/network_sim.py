...
class Network:
    ...
    def queue_initial_packets(self):
        for sender in self.senders:
            ...
            sender.reset_obs()
            heapq.heappush(
                self.q, (1.0 / sender.rate, sender, EVENT_TYPE_SEND, 0, 0.0, False)
            )
...
class SimulatedNetworkEnv(gym.Env):
    def __init__(
        ...
        self.min_loss, self.max_loss = (0.0, 0.02)
        self.history_len = history_len
    ...
    def _get_all_sender_obs(self):
        ...
        sender_obs = np.array(sender_obs).reshape(
            -1,
        )
        # print(sender_obs)
        return sender_obs
    ...
    def step(self, actions):
        ...
        return (
            sender_obs,
            reward,
            (self.steps_taken >= self.max_steps or should_stop),
            {},
        )
